...
int interpolationSearch(long arr[], long l, long r, long x)
{
    long low = 0;
    long high = r - 1;
    while (arr[high] >= x && x > arr[low]){
    long next = low + (x-arr[low])/(arr[high]-arr[low])*(high-low);
    
  if(x > arr[next])low = next+1;
     else if(x<arr[next])high = next-1;
       else low = next;
...
int main() {
    ...
    int index=interpolationSearch(ints,0,size-1,d);
